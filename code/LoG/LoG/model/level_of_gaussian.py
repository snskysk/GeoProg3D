...
class LoG(nn.Module):
    ...
    def step(self):
        ...
        ##
        # params = params.pop('colors')
        # print(index.shape)
        # print(flag_vis.shape)
        # print(params.keys)
        # sys.exit()
    ...
    def update_init_stage(self, scale=1, cur_stage_name="init"):
        ...
        if mode == 'split_by_2d':
            ...
            # print(f'[{self.__class__.__name__}] {self.counter.str_min_mean_max("radii_split", radii_max[flag_split])}')## add comment out
        ...
        radius = self.gaussian.activation.scaling_activation(self.gaussian.scaling.detach())
        radius_max = radius.max(dim=-1).values
        # print(f'[{self.__class__.__name__}] {self.counter.str_min_mean_max("radius3d_min", self.counter.radius3d_min)}')## add comment out
        self.counter.reset(self.num_points)

    def update_depth_stage(self, global_iteration, cur_stage_name="init"):
    ...
    def update_by_iteration(self, iteration, global_iteration, cur_stage_name="init"):
        ...
        if (iteration + 1 > densify_from_iter) and (iteration + 1) % densify_every_iter == 0:
            ...
            if self.current_depth == 0:
                if self.stage_name == 'init':
                    self.update_init_stage(cur_stage_name=cur_stage_name)
                else:
                    self.update_init_stage(scale=2, cur_stage_name=cur_stage_name)
            else:
                if (iteration + 1) % (2*densify_every_iter) == 0:
                    self.update_depth_stage(global_iteration, cur_stage_name=cur_stage_name)
                else:
                    self.counter.reset(self.num_points)
            return True
        return False
