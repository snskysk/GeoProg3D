...
class Trainer(nn.Module):
    ...
    def resize_np_array(self, np_array, target_size):
        ...
        # Stack the resized array
        return np.stack(resized_list)


    def get_language_feature(self, language_feature_dir, feature_level, batch, dir_structure, lang_feat_name):
        image_height = batch["camera"]["image_height"].cpu().numpy()[0]
        image_width = batch["camera"]["image_width"].cpu().numpy()[0]

        if dir_structure == "single":
            # to get ex DJI_0001 from path/to/DJI_0001.ext
            image_name = os.path.basename(batch["imgname"][0]).split(".")[0]
        elif dir_structure == "multi":
            # to get ex 110MEDIA-DJI_0001 from path/to/cam(110MEDIA)/DJI_0001.ext
            full_path_name_wo_ext = batch["imgname"][0].split(".")[0]
            target_elements_of_fpn_wo_ext = full_path_name_wo_ext.split("/")[-2:]
            image_name = "-".join(target_elements_of_fpn_wo_ext)

        language_feature_name = os.path.join(language_feature_dir, image_name)
        seg_map_np = np.load(language_feature_name + '_s.npy')
        assert seg_map_np.ndim == 3
        seg_dims, seg_h, seg_w = seg_map_np.shape

        seg_map = torch.from_numpy(seg_map_np)# torch.Size([4, 730, 988]) case frame_00157_s.npy
        feature_map = torch.from_numpy(np.load(language_feature_name + '_f.npy'))# torch.Size([136, 3]) case frame_00157_f.npy
        
        after_fix = True
        if after_fix:
            mask_height, mask_width = seg_map.shape[1], seg_map.shape[2]
            y, x = torch.meshgrid(torch.arange(0, mask_height), torch.arange(0, mask_width))# y = torch.Size([730, 988]), x = torch.Size([730, 988])
            x = x.reshape(-1, 1)# x = torch.Size([721240, 1])
            y = y.reshape(-1, 1)# y = torch.Size([721240, 1])
            seg = seg_map[:, y, x].squeeze(-1).long()# from torch.Size([4, 730, 988]) to torch.Size([4, 721240])
            # print("seg:", seg.shape)

            mask = seg != -1# torch.Size([4, 721240])
            if feature_level == 0: # default
                point_feature1 = feature_map[seg[0:1]].squeeze(0)
                mask = mask[0:1].reshape(1, mask_height, mask_width)
            elif feature_level == 1: # s
                point_feature1 = feature_map[seg[1:2]].squeeze(0)
                mask = mask[1:2].reshape(1, mask_height, mask_width)
            elif feature_level == 2: # m
                point_feature1 = feature_map[seg[2:3]].squeeze(0)
                mask = mask[2:3].reshape(1, mask_height, mask_width)
            elif feature_level == 3: # l
                point_feature1 = feature_map[seg[3:4]].squeeze(0)
                mask = mask[3:4].reshape(1, mask_height, mask_width)
            else:
            ...
            point_feature = point_feature1.reshape(mask_height, mask_width, -1).permute(2, 0, 1)

            # Size after resizing.
            target_size = (image_height, image_width)
            # Executing resize
            point_feature = F.interpolate(point_feature.unsqueeze(0), size=target_size, mode='bilinear', align_corners=False).squeeze(0)
            mask = mask.float()
            mask = F.interpolate(mask.unsqueeze(0), size=target_size, mode='bilinear', align_corners=False).squeeze(0)
            mask = mask > 0.5

        else:
            y, x = torch.meshgrid(torch.arange(0, image_height), torch.arange(0, image_width))# y = torch.Size([730, 988]), x = torch.Size([730, 988])
            x = x.reshape(-1, 1)# x = torch.Size([721240, 1])
            y = y.reshape(-1, 1)# y = torch.Size([721240, 1])
            seg = seg_map[:, y, x].squeeze(-1).long()# from torch.Size([4, 730, 988]) to torch.Size([4, 721240])
            # print("seg:", seg.shape)
    ...
    def training_step(self, model, data, step=True, accumulate_step=1, lang_gt_config={"stage":"tree", "language_feature_dir":"/workspace/t2024/LangSplat/data/citygen/US-NewYork-Queens-10-Center-Blvd-R375-A410/language_features_dim3/", "feature_level":0, "dir_structure":"single"}):
        batch = prepare_batch(data, self.device)

        if lang_gt_config["stage"] == "tree_colors":
            # This is based on the assumption that batch_size=1.
            batch_shape = batch["image"].shape# torch.Size([1, 538, 959, 3])

            file_basename_wo_ext = os.path.basename(batch["imgname"][0]).split(".")[0]
            img_height = batch["camera"]["image_height"]
            img_width = batch["camera"]["image_width"]
            language_feature_dir = lang_gt_config["language_feature_dir"]
            feature_level = lang_gt_config["feature_level"]
            dir_structure = lang_gt_config["dir_structure"]
            lang_feat_name = lang_gt_config["lang_feat_name"]

            gt_language_feature, language_feature_mask = self.get_language_feature(language_feature_dir, feature_level, batch, dir_structure, lang_feat_name)
            new_batch_image = gt_language_feature*language_feature_mask
            new_batch_image = torch.permute(new_batch_image, (1, 2, 0))
            new_batch_image = torch.unsqueeze(new_batch_image, 0)
            batch["image"] = new_batch_image
    ...
    def init(self, dataset):
        ...
        if self.overlook is not None:
            ...
            self.model.train()
        self.model.gaussian.colors.shape
    ...
    def fit(self, dataset):
        ...
        for stage_name, stage in self.cfg.train.stages.items():
            ...
            # print("optimize_keys:", self.model.optimizer.optimize_keys)
            # self.model.optimizer.optimize_keys = ["xyz", "colors", "scaling", "opacity", "rotation", "shs"]
            # print("optimize_keys:", self.model.optimizer.optimize_keys)

            language_feature_dir = self.cfg.language_feature_path
            feature_level = self.cfg.sam_feature_level
            try:
                dir_structure = self.cfg.dir_structure
            except:
                dir_structure = "single"

            if stage_name in ["init"]:            
                # Coordinates: The initial weight is gradually moved by a random value.
                # self.model.gaussian.xyz += torch.rand(self.model.gaussian.xyz.size()).to(self.device)
                self.model.optimizer.optimize_keys = ["xyz", "colors", "scaling", "opacity", "rotation", "shs"]
            if stage_name in ["tree"]:            
                self.model.optimizer.optimize_keys = ["xyz", "colors", "scaling", "opacity", "rotation", "shs"]
            if stage_name in ["tree_colors"]:            
                # self.model.optimizer.optimize_keys = ["xyz", "scaling", "opacity", "rotation", "shs"]
                self.model.optimizer.optimize_keys = ["colors"]
                print("optimize_keys:", self.model.optimizer.optimize_keys)
                # Color: Create a new matrix (fill all values with the average value)
                self.model.gaussian.colors = torch.full(self.model.gaussian.colors.size(), self.model.gaussian.colors.mean()).to(self.device)

            lang_gt_config = {"stage":stage_name, "language_feature_dir":language_feature_dir, "feature_level":feature_level, "dir_structure":dir_structure, "lang_feat_name":""}

            real_iter = 0
            previous_batch = None
            last_lang_iter = 0
            for iteration, data in enumerate(trainloader):
                print("\riteration:{}, real_iter:{}".format(iteration, real_iter), end="")
                if stage_name in ["tree_colors"] and dir_structure == "multi":            
                    batch = prepare_batch(data, self.device)
                    ...
                    image_name = "-".join(target_elements_of_fpn_wo_ext)
                    lang_feat_name = os.path.join(language_feature_dir, image_name)
                    if not os.path.exists(lang_feat_name + '_s.npy'):
                        self.global_iterations += 1
                        continue
                self.model.clear()
                self.render.iteration = self.global_iterations
                flag, output, loss = self.training_step(self.model, data, lang_gt_config=lang_gt_config)
                if not flag:
                    self.global_iterations += 1
                    continue
                ...
                with torch.no_grad():
                    if (iteration + 1) < len(trainloader):
                        ...
                        flag_update = self.model.update_by_iteration(iteration, self.global_iterations, cur_stage_name=stage_name)
                        if flag_update:
                            need_log = True
                            if self.overlook is not None and self.check_iteration(stage_name, iteration + 1, self.cfg.overlook.iteration):
